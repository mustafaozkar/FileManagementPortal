...
namespace Core.Utilities.Security.JWT.Concrete
{
    public class JwtHelper : ITokenHelper
    {
        ...
        public AccessToken CreateToken(User user, List<OperationClaim> operationClaims)
        {
            ...
            var jsonWebToken = CreateJwtSecurityToken(_tokenOptions, user, signingCredentials, operationClaims);
            var jsonSecurityTokenHandler = new JwtSecurityTokenHandler();
            var token = jsonSecurityTokenHandler.WriteToken(jsonWebToken);
        ...
        private IEnumerable<Claim> SetClaims(User user, List<OperationClaim> operationClaims)
        {
            ...
            claims.AddRoles(operationClaims.Select(operationClaim => operationClaim.Name).ToList());
            return claims;
        }
    }
}
